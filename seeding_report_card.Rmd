...
#fit beverton-holt model 2 with separate hatchery and wild productivity & separate capacity, BUT where difference in prod and cap are same 
# nll_bh2<-function(par,P,R,S){
#   logalpha<-par[1]
#   alpha=exp(logalpha)
#   logbeta<-par[2]
#   beta<-exp(logbeta)
...
#   logsigma<-par[4]
#   sigma<-exp(logsigma)
#   R_hat<-(alpha * (1-P) * S + alpha * b * P * S)/(1 + (alpha * (1-P) * S + alpha * b * P * S) / (b * P * beta + (1-P) * beta)) 
#   resid<-log(R_hat)-log(R)
#   nll=sum(-1*log(dnorm(resid,mean = 0,sd = sigma)))
# }
...
pacman::p_load(tidyverse,RColorBrewer,kableExtra,gridExtra,viridis)
```
...
fits2%>%
  filter(P%in%c(0,1))%>%
  pivot_longer(names_to = "model",values_to = "R",cols=c("Ricker",`Ricker version 2`, "Beverton-Holt"))%>%
  pivot_wider(names_from = P, values_from = R)%>%
  mutate(RRS=(`0`/S)/(`1`/S))%>%
  ggplot(aes(x=S,y=RRS,color=model))+
